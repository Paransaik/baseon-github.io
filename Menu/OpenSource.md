...
시험문제 25문제(>30문제) 시간 30분(객관식10, 짧은 주관식12~13, 약간 긴 주관식3~4개)
...
31. 83p-깃허브에 퍼블릭 키 전송하기
 ...
 - setting > 오른쪽 SSH and GPG Keys클릭하고 퍼블릭 키를 추가한다.
   * 지역 저장소에서 만든 키 가운데 퍼블릭 키를 깃허브로 전송한 다음 저장
   * 깃허브 접속 시 사용자 컴퓨터의 프라이빗 키와 깃허브 서브의 퍼블릭 키를 비교
   * 퍼블릭 키와 프라이빗 키는 한쌍이므로 두 개의 키가 서로 맞으면 사용자 컴퓨터와 깃허브 저장소가 연결
...
34. 93p-해결책
 - 처음에 저장했던 저장소 전체를 여러 개 복사
 - 각 고객사의 이름을 붙임
 - 각 고객사 저장소마다 버전 관리를 따로한다.
...
36. 99p-브랜치 사이 이동하기
 - git checkout
   * vim work.txt
   * git commit -am "master contents 4"
   * git log --oneline★
   * git log --oneline --branches --graph: 각 브랜치의 커밋을 그래프로 확인★
 
37. 106p-브랜치 병합하는 3가지 방법★
 - 서로 다른 파일 병합하기
   * git init manual-2: 새로운 디렉토리를 만들고 젖아소를 초기화
 - 같은 파일의 다른 위치 수정시 병합
 - 같은 파일의 같은 위치 수정시 병합(comflict)
...
42. 135p-git fetch
 ...
 - pull은 원격저장소의 커밋을 가져와서 무조건 지역 저장소와 합침
   * cd ~/git_office
   * git fetch: 원격 저장소로부터 무언가를 가져옴
   * 원격 저장소의 최신 커밋 정보를 가져왔지만 아직 지역 저장소를 합치지 않아 보이지 않음
